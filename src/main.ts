...
import { BASIS_scene } from "./BASIS/3.scene";
import { BASIS_entity } from "./BASIS/4.entity";
import { BASIS_label } from "./BASIS/5.label";
...
export = (gameMainParam: g.GameMainParameterObject) => {
   ...
   * 
   * `npm run dev` で実行している場合は保存「CTRL S」するたびに再実行されます
   * ※極稀にタブの状態が壊れます. その場合はブラウザのタブリロードを行ってください
   *   リロードで解決しない場合は `npm run dev` を再実行してください
   * 
  ...
  // 【基本編】
  if (true) BASIS_minimal();       // 1. マルチプレイの小さいサンプル
  if (false) BASIS_game();          // 2. g.game について
  if (false) BASIS_scene();         // 3. シーン (g.Scene)
  if (false) BASIS_entity();        // 4. エンティティ (g.E)
  ...
  if (false) MULTI_global_event();  // 12. プレイヤー間での操作の共有
